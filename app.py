...
def check_recaptcha(resp, ip):
    try:
        url = URL('https://www.google.com/recaptcha/api/siteverify?secret=%s&response=%s&ip=%s' % (settings.RE_SECRET, resp, ip))
...
def handler():
    ...
    if not cookie_name or not cookie_val:
        abort(500)
    ip = get_client_addr()
    if check_recaptcha(request.form['g-recaptcha-response'], ip):
